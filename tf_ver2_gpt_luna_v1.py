...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def matmul_prefix_sum(self, x, y, z, yz_prev=0.0):
        batch_size = tf.shape(x)[0]
        seq_length = tf.shape(x)[2]
        one_vector = tf.ones(
            [batch_size, 1], 
        ...
        yz_prefix  = tf.add(
            yz_prev, tf.math.cumsum(yz, axis=2))
        xyz_causal = tf.squeeze(
            tf.matmul(x, yz_prefix), axis=3)
        
        yz_curr = yz_prefix[:, :, -1:, :, :]
        return yz_curr, xyz_causal
    
    def call(
        self, v, k, q, p, 
        yz_prev_u=0.0, yz_prev_o=0.0):
        ...
        attn_tuple_1 = self.matmul_prefix_sum(
            q, k, attn_pack, yz_prev=yz_prev_u)
        attn_unpack  = tf.nn.softmax(attn_tuple_1[1])
        
        attn_tuple_2 = self.matmul_prefix_sum(
            attn_unpack, attn_pack, v, yz_prev=yz_prev_o)
        attn_outputs = self.wc(
            self.combine_heads(attn_tuple_2[1]))
        
        yz_curr_u = attn_tuple_1[0]
        yz_curr_o = attn_tuple_2[0]
        return yz_curr_u, yz_curr_o, attn_outputs
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(
        self, x_enc, x_p, x_pos, 
        yz_prev_u=0.0, yz_prev_o=0.0, training=True):
        x_embed = x_enc + x_pos
        
        # Decoder only Attention Mechanism. #
        attn_tuple = self.attn_self(
            x_embed, x_embed, x_embed, x_p, 
            yz_prev_u=yz_prev_u, yz_prev_o=yz_prev_o)
        yz_curr_u  = attn_tuple[0]
        yz_curr_o  = attn_tuple[1]
        
        attn_self_output = self.dropout_1(
            attn_tuple[2], training=training)
        ...
        return yz_curr_u, yz_curr_o, ffwd_self_output
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(
        self, x, l_st=0, yz_prev_u=None, 
        yz_prev_o=None, training=True):
        batch_size = tf.shape(x)[0]
        seq_length = tf.shape(x)[1]
        one_vector = tf.ones(
            [batch_size, 1], 
        ...
        if yz_prev_u is None:
            yz_prev_u = [0.0 for _ in range(self.n_layers)]
        if yz_prev_o is None:
            yz_prev_o = [0.0 for _ in range(self.n_layers)]
        ...
        for m in range(self.n_layers):
            ...
            output_tuple = self.dec_layers[m](
                ...
                x_pos_embed, yz_prev_u=yz_prev_u[m], 
                yz_prev_o=yz_prev_o[m], training=training)
        ...
        yz_curr_u = tf.concat(yz_curr_u, axis=0)
        yz_curr_o = tf.concat(yz_curr_o, axis=0)
        return yz_curr_u, yz_curr_o, output_tuple[2]
...
class GPTLuna(tf.keras.Model):
    ...
    def call(
        ...
        output_tuple = self.gpt_model(
            x, l_st=l_st, yz_prev_u=yz_prev_u, 
            yz_prev_o=yz_prev_o, training=training)
        
        yz_curr_u = output_tuple[0]
        yz_curr_o = output_tuple[1]
        dec_outputs = output_tuple[2]
        dec_logits  = self.p_decoder(dec_outputs)
        return yz_curr_u, yz_curr_o, dec_logits
    ...
    def infer_v1(self, x):
        ...
        for step in range(self.seq_len):
            ...
            tmp_tuple  = self.call(
                tmp_inputs[:, -1:], 
                l_st=step, yz_prev_u=yz_prev_u, 
                yz_prev_o=yz_prev_o, training=False)
            
            yz_prev_u = tmp_tuple[0]
            yz_prev_o = tmp_tuple[1]
            tmp_logit = tmp_tuple[2][:, -1, :]
