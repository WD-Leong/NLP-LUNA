...
def sub_batch_train_step(
    ...
    if input_len <= l_window:
        n_windows = 1
    elif input_len % l_window == 0:
        n_windows = int(input_len / l_window)
    else:
        n_windows = int(input_len / l_window) + 1
    ...
    for n_sub in range(sub_batch):
        ...
        for n_win in range(n_windows):
            l_st = n_win*l_window
            if n_win == (n_windows - 1):
                l_en = input_len
            else:
                l_en = (n_win+1)*l_window
            
            tmp_encode = x_encode[id_st:id_en, l_st:l_en]
            tmp_output = x_output[id_st:id_en, l_st:l_en]
            
            with tf.GradientTape() as grad_tape:
                output_tuple  = model(
                    tmp_encode, l_st=l_st, 
                    yz_prev_u=yz_prev_u, 
                    yz_prev_o=yz_prev_o, training=True)
                
                yz_prev_u = output_tuple[0]
                yz_prev_o = output_tuple[1]
                output_logits = output_tuple[2]
...
model_ckpt_dir  = "TF_Models/dialogue_sw_gpt_luna_v1"
train_loss_file = "train_loss_dialogue_sw_gpt_luna_v1.csv"
...
# Build the GPT. #
print("Building the GPT LUNA Model.")
start_time = time.time()

gpt_model = gpt.GPTLuna(
    num_layers, num_heads, 
    hidden_size, ffwd_size, vocab_size, 
    seq_length, p_len, rate1=0.0, rate2=p_drop)
...
print("-" * 50)
print("Training the GPT LUNA Network", 
...
while n_iter < maximum_iter:
    ...
    tmp_loss = sub_batch_train_step(
        gpt_model, sub_batch, 
        window_len, tmp_input, tmp_output, 
    ...
    if n_iter % display_step == 0:
        ...
        tmp_i_tok = bpe.bp_decode(
            tmp_i_idx, idx_2_subword)
        tmp_o_tok = bpe.bp_decode(
            tmp_data[1], idx_2_subword)
